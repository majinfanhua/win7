            ...
            {section === 'ai' && (
              <section className="card">
                ...
                {/*
                  ...
                  **默认不勾**（不校验），因为它在这个应用的目标平台上
                  基本是坏的：Win7 的根证书库随系统更新，而 Win7 早已
                  停止主流支持，新根 CA（如 Let's Encrypt 的 ISRG Root X1）
                  根本装不进去 —— 而中转站用免费证书的非常多。
                  勾上之后 Chromium 会判定链不可信、直接拒连，
                  用户换多少个中转站都没用，只会以为「这软件连不上网」。

                  所以语义是「我想额外开启校验」，而不是「我允许不安全」：
                  前者用户看不懂可以不勾，后者看起来像在推荐你去关安全。

                  代价必须写清楚 —— 不校验时无法确认对端身份。
                */}
                <div className="field">
                  ...
                  <div className="hint">
                    <strong>默认不勾</strong>，也就是不验证证书。
                    这是因为 Win7 的根证书库太旧、装不进新的根 CA
                    （比如 Let's Encrypt），勾上之后很可能
                    <strong>直接连不上中转站</strong>
                    ，报 <code>ERR_CERT_AUTHORITY_INVALID</code>。
                    <br />
                    如果你的系统是新装的 Win10/11、或者打过补丁，
                    可以勾上来换回这道防护：
                    <strong>不校验时无法确认对面就是那个中转站</strong>
                    ，密钥与对话内容可能被中间人截获
                    （公共 WiFi、校园网代理之类）。
                  </div>
                </div>
